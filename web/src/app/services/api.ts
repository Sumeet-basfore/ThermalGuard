...
export class ApiService {
  ...
  public static async getSensors(): Promise<{
    ...
    if (this.mode === "live") {
      try {
        ...
        const response = await fetch(`${activeEndpoint}/sensors`, {
  ...
  public static async getThermalFrame(): Promise<ThermalFrameData> {
    if (this.mode === "live") {
      try {
        ...
        const response = await fetch(`${activeEndpoint}/thermal`, {
          signal: controller.signal,
        });
        clearTimeout(timeoutId);
  ...
  public static async getHealth(): Promise<SystemHealthMetrics> {
    if (this.mode === "live") {
      try {
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), this.timeoutMs);
        const response = await fetch(`${activeEndpoint}/health`, { signal: controller.signal });
        clearTimeout(timeoutId);
  ...
  public static async saveSettings(settings: SystemSettings): Promise<boolean> {
    if (this.mode === "live") {
      ...
      const response = await fetch(`${activeEndpoint}/settings`, {
        method: "POST",
