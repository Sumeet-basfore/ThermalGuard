...
function Panel({
  ...
  return (
    <section className={`rounded-lg border border-[#2A3140] bg-[#151922] p-5 shadow-sm transition-colors duration-150 ${className}`}>
      {children}
    </section>
  );
}
    ...
    <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
      <div>
        <div className="flex items-center gap-2">
          ...
          <p className="font-mono text-xs font-medium text-[#64748B]">
            ESP32 Node #192.168.1.48 · Substation B
          </p>
        </div>
        <h2 className="mt-1 font-sans text-32px font-bold tracking-tight text-[#F8FAFC]">
          {title}
        </h2>
        <p className="mt-1 text-sm text-[#94A3B8] font-normal">{description}</p>
      </div>
...
function Shell() {
  ...
  return (
    <div className="min-h-screen bg-[#0B0D12] font-[Inter] text-[#F8FAFC]">
      <SidebarNav
        navItems={nav}
      ...
      <main className="lg:pl-[240px]">
        ...
        <footer className="mx-6 mb-6 flex flex-wrap items-center gap-x-6 gap-y-2 border-t border-[#2A3140] pt-4 font-mono text-xs text-[#64748B] lg:mx-8">
          <span>
            ESP32 Firmware <b className="font-semibold text-[#94A3B8]">v2.4.1</b>
          </span>
          <span>•</span>
          <span>
    ...
    <>
      ...
      <div className="rounded-lg border border-[#2A3140] bg-[#151922] p-6 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-6">
          ...
          <div className="rounded border border-[#2A3140] bg-[#0B0D12] p-3">
    ...
    <>
      ...
      <Panel className="overflow-hidden">
        <div className="grid lg:grid-cols-[minmax(0,1fr)_280px]">
          ...
          <div className="border-t border-[#2A3140] bg-[#0B0D12] p-5 lg:border-l lg:border-t-0 font-mono text-xs">
            <p className="font-semibold text-[#94A3B8] uppercase">
              Thermal Calibration Scale
            </p>
            <div className="mt-2 h-2.5 rounded bg-gradient-to-r from-blue-900 via-cyan-400 to-rose-600" />
            <div className="mt-1 flex justify-between text-[#64748B] font-semibold text-[10px]">
              <span>18°C</span>
              <span>31°C</span>
              <span>45°C</span>
            </div>

            <p className="mt-6 font-semibold text-[#94A3B8] uppercase">
              Array Telemetry
            </p>
            <dl className="mt-3 space-y-3">
              {[
                ...
                ["Mean Grid Temp", "29.6 °C", "text-[#F8FAFC]"],
                ["Hotspot Index", "X: 22, Y: 14", "text-[#06b6d4]"],
                ["Frame Rate", "8.0 FPS", "text-[#16A34A]"],
              ].map(([key, val, valColor]) => (
                <div className="flex justify-between items-center border-b border-[#2A3140]/50 pb-2" key={key}>
                  <dt className="text-[#94A3B8] font-sans">{key}</dt>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {cards.map((c) => (
          <Panel key={c.label}>
            ...
            <p className="mt-2 font-mono text-32px font-bold tracking-tight text-[#F8FAFC]">{c.val}</p>
            <div className="mt-2">
              <TrendIndicator
                value={c.delta}
                label="vs baseline"
  ...
  const alertsList = [
    {
      ...
      time: "09:42:18",
      badgeTone: "bg-[#DC2626]/10 text-[#DC2626] border-[#DC2626]/30",
      iconTone: "bg-[#DC2626]/10 text-[#DC2626]",
    },
    {
      level: "Warning",
    ...
    },
    {
      ...
      time: "Yesterday",
      badgeTone: "bg-[#2563EB]/10 text-[#2563EB] border-[#2563EB]/30",
      iconTone: "bg-[#2563EB]/10 text-[#2563EB]",
    },
  ];
      ...
      <PageHead
        ...
        action={
          <button
            ...
            className="flex items-center gap-2 rounded border border-[#2A3140] bg-[#151922] px-3.5 py-1.5 text-xs font-semibold text-[#F8FAFC] hover:bg-[#2A3140] transition-colors"
          >
      ...
      <Panel className="overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[700px] text-left text-xs">
            <thead className="border-b border-[#2A3140] bg-[#0B0D12] font-mono text-[11px] uppercase tracking-wider text-[#64748B]">
              <tr>
            ...
            <tbody className="divide-y divide-[#2A3140]/50 font-mono">
              {rows.map((row, i) => (
                <tr className="hover:bg-[#0B0D12]/50 transition-colors" key={i}>
                  <td className="px-5 py-3 text-[#F8FAFC] font-semibold">{row[0]}</td>
                  ...
                  <td className="px-5 py-3 text-[#94A3B8]">{row[4]}</td>
  ...
  return (
    <>
      ...
      <Panel className="max-w-2xl">
        <h3 className="text-base font-bold text-[#F8FAFC]">Hardware Trip Thresholds</h3>
        ...
        <div className="mt-6 grid gap-5 sm:grid-cols-2">
          <label className="block">
            <span className="text-xs font-medium text-[#94A3B8]">Hotspot Trip Temperature</span>
            <div className="mt-1.5 flex rounded border border-[#2A3140] bg-[#0B0D12] px-3 py-2">
              <input
                value={tempThresh}
                onChange={(e) => setTempThresh(e.target.value)}
                className="w-full bg-transparent font-mono text-sm font-bold text-[#F8FAFC] outline-none"
              />
              <span className="font-mono text-xs font-medium text-[#64748B]">°C</span>
            </div>
          </label>

          <label className="block">
            <span className="text-xs font-medium text-[#94A3B8]">Overcurrent Limit (ACS712)</span>
            <div className="mt-1.5 flex rounded border border-[#2A3140] bg-[#0B0D12] px-3 py-2">
              <input
                value={currLimit}
                ...
                className="w-full bg-transparent font-mono text-sm font-bold text-[#F8FAFC] outline-none"
              />
              <span className="font-mono text-xs font-medium text-[#64748B]">A</span>
            </div>
          </label>

          <label className="block">
            <span className="text-xs font-medium text-[#94A3B8]">Buzzer Alarm Delay</span>
            <div className="mt-1.5 flex rounded border border-[#2A3140] bg-[#0B0D12] px-3 py-2">
              <input
                value={alarmDel}
                ...
                className="w-full bg-transparent font-mono text-sm font-bold text-[#F8FAFC] outline-none"
              />
              <span className="font-mono text-xs font-medium text-[#64748B]">sec</span>
            </div>
          </label>

          <label className="block">
            <span className="text-xs font-medium text-[#94A3B8]">Relay Disconnect Delay</span>
            <div className="mt-1.5 flex rounded border border-[#2A3140] bg-[#0B0D12] px-3 py-2">
              <input
                value={relayDel}
    ...
    <>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.4fr_0.6fr]">
        <Panel hover={false}>
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-3">
            {[
              ...
              <div className="rounded border border-[#2A3140] bg-[#0B0D12] p-3.5" key={a}>
                <p className="font-mono text-xs font-bold text-[#2563EB]">{a}</p>
                <p className="mt-1 text-xs text-[#94A3B8] font-medium">{b}</p>
              </div>
            ))}
          </div>
        </Panel>
        ...
        <Panel hover={false}>
          ...
          <p className="mt-2 font-mono text-32px font-bold text-[#F8FAFC]">v2.4.1</p>
          <p className="mt-1 text-xs text-[#94A3B8] font-mono">PlatformIO Core 6.1.11</p>
          <Link
            to="/devices"
...
function NotFound() {
  return (
    <div className="grid min-h-[60vh] place-items-center p-6">
      <div className="max-w-md rounded-lg border border-[#2A3140] bg-[#151922] p-8 text-center shadow-sm">
        <div className="mx-auto grid h-12 w-12 place-items-center rounded border border-[#D97706]/30 bg-[#D97706]/10 text-[#D97706]">
          <ShieldAlert size={24} />
        </div>
        <p className="mt-4 font-mono text-32px font-bold text-[#F8FAFC]">
          404
        </p>
        <h2 className="mt-1 text-base font-bold text-[#F8FAFC]">Unknown Route Path</h2>
        ...
        <div className="mt-6 flex flex-col gap-2">
          ...
          <Link
            to="/thermal-monitor"
            className="rounded border border-[#2A3140] bg-[#0B0D12] py-2 text-xs font-semibold text-[#94A3B8] hover:text-[#F8FAFC] transition-colors"
          >
